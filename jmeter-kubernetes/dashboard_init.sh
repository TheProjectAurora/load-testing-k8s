...
echo "Add infludb datasource:"
kubectl exec -ti -n $tenant $master_pod -- curl 'http://admin:admin@jmeter-grafana:3000/api/datasources' -X POST -H 'Content-Type: application/json;charset=UTF-8' --data-binary '{"name":"jmeterdb","type":"influxdb","url":"http://jmeter-influxdb:8086","access":"proxy","isDefault":true,"database":"jmeterdb","user":"admin","password":"admin"}'
