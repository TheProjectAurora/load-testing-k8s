...
```
./start_test.sh cluster test_nginx.jmx http://nginx.suteg:80 jmeter_results.csv -GTHREADS=20 -GRAMP_UP_SEC=60 -GDURATION_SEC=60 -GDEVIATION=50 -GCONSTANT_DELAY=100 -GSETUP_THREADS=10 -GSETUP_RAMP_UP_SEC=60
```
### Execute jmeter in master mode: 
```
./start_test.sh master test_nginx.jmx http://nginx.suteg:80 jmeter_results.csv -JTHREADS=20 -JRAMP_UP_SEC=60 -JDURATION_SEC=60 -JDEVIATION=50 -JCONSTANT_DELAY=100 -JSETUP_THREADS=10 -JSETUP_RAMP_UP_SEC=60
```
...
jmeter -n -t test_nginx.jmx -JBASE_URL=http://localhost:80 -l jmeter_results.csv -JTHREADS=10 -JRAMP_UP_SEC=60 -JDURATION_SEC=60 -JDEVIATION=50 -JCONSTANT_DELAY=100 -JSETUP_THREADS=10 -JSETUP_RAMP_UP_SEC=60
```
### Against nginx.suteg that is kicked up in docker-compose.yaml
```
jmeter -n -t test_nginx.jmx -JBASE_URL=http://nginx.sut:80 -l jmeter_results.csv -JTHREADS=10 -JRAMP_UP_SEC=60 -JDURATION_SEC=60 -JDEVIATION=50 -JCONSTANT_DELAY=100 -JSETUP_THREADS=10 -JSETUP_RAMP_UP_SEC=60
```
