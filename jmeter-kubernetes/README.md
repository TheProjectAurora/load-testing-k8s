...
```
./start_test.sh cluster test_nginx.jmx -GBASE_URL=http://nginx.suteg:80 jmeter_results.csv -GTHREADS=50 -GRAMP_UP_MIN=1 -GHOLD_TARGET_MIN=0 -GDEVIATION=50 -GCONSTANT_DELAY=100
```
### Execute jmeter in master mode: 
```
./start_test.sh master test_nginx.jmx -JBASE_URL=http://nginx.suteg:80 jmeter_results.csv -JTHREADS=50 -JRAMP_UP_MIN=1 -JHOLD_TARGET_MIN=0 -JDEVIATION=50 -JCONSTANT_DELAY=100
```
...
jmeter -n -t test_nginx.jmx -JBASE_URL=http://localhost:80 -l jmeter_results.csv -JTHREADS=50 -JRAMP_UP_MIN=1 -JHOLD_TARGET_MIN=0 -JDEVIATION=50 -JCONSTANT_DELAY=100
```
### Against nginx.suteg that is kicked up in docker-compose.yaml
```
jmeter -n -t test_nginx.jmx -JBASE_URL=http://nginx.sut:80 -l jmeter_results.csv -JTHREADS=50 -JRAMP_UP_MIN=1 -JHOLD_TARGET_MIN=0 -JDEVIATION=50 -JCONSTANT_DELAY=100
```
...
kubectl delete namespace <same_that_you_give_to_cluster>
